...
def main():
    ...
    print("=" * 60)
    print("第三轮测试：修正对冲方向逻辑")
    print("=" * 60)
    print(f"开始时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()
    
    # 加载测试信号
    signals_file = data_dir / "test_signals_round3.json"
    
    if not signals_file.exists():
        print("❌ 测试信号文件不存在")
        print("请先运行: python3 generate_test_signals_round3.py")
        return
    ...
    report_file = data_dir / "test_report_round3.json"
